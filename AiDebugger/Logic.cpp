...
namespace DS14
{
	...
	void HeroMove(Status & gameState,PlayerCommand * command1,PlayerCommand * command2)
	{
		...
		for(int i=0;i<3;i++)
		{
                ...
                isOperablew(gameState.AI1HeroInfo[i]))
			{
				...
				if(gameState.AI1HeroInfo[i].hasPot) movespeed = movespeed*(1-double(POT_REDUCE)/100); //根据带香锅情况减去一定速度
				//darkthecross 修改：potreduce减少的是百分比
				...
				{
					target = check_intersection(gameState.potInfo.groundPots[j],gameState.AI1HeroInfo[i].heroPosition,target,2*HERO_RADIUS);//根据香锅刷新target
				}
		...
		for(int i=0;i<3;i++) //另一方
		{
                ...
                isOperablew(gameState.AI2HeroInfo[i]))
			{
				...
				if(gameState.AI2HeroInfo[i].hasPot) movespeed = movespeed*(1-double(POT_REDUCE)/100); //根据带香锅情况减去一定速度
				//darkthecross 修改：potreduce减少的是百分比
				//Han再次修改：减少百分比放在所有的加、减速最后
				Coordinate target;
				...
				for(int j=0;j<gameState.potInfo.groundNumber;j++)
				{
					target = check_intersection(gameState.potInfo.groundPots[j],gameState.AI2HeroInfo[i].heroPosition,target,2*HERO_RADIUS);//根据香锅刷新target
				}
				Coordinate o_center;
				o_center.x=o_center.y=0;//o_center表示原点
				if(Distance(target,o_center)>MAP_RADIUS-HERO_RADIUS)
        ...
        for(int i=0;i<3;i++)
        {
            ...
            if(command1->heroOrder[i]==fastWalk)
            {
                if(gameState.AI1HeroInfo[i].StunLast==0&&           //英雄未眩晕
                    ...
                    !gameState.AI1HeroInfo[i].hasPot&&				//英雄没拿香锅
        ...
        for(int i=0;i<3;i++) //另一玩家
        {
            ...
            if(command2->heroOrder[i]==fastWalk)
            {
                if(gameState.AI2HeroInfo[i].StunLast==0&&           //英雄未眩晕
                    ...
                    !gameState.AI2HeroInfo[i].hasPot&&				//英雄没拿香锅
	...
	void HeroStun(Status & gameState,PlayerCommand * command1,PlayerCommand * command2)
	{
		for(int i=0;i<3;i++)
		{
			...
			if(gameState.AI1HeroInfo[i].StunLast==0&&              //英雄未眩晕
				gameState.AI1HeroInfo[i].RebornWait==0&&           //英雄未死亡
				!gameState.AI1HeroInfo[i].hasPot&&                 //英雄未拿香锅
		...
		for(int i=0;i<3;i++) //另一方
		{
			...
			if(gameState.AI2HeroInfo[i].StunLast==0&&              //英雄未眩晕
				gameState.AI2HeroInfo[i].RebornWait==0&&           //英雄未死亡
				!gameState.AI2HeroInfo[i].hasPot&&                 //英雄未拿香锅
    ...
    void HeroMissile(Status &gameState,const PlayerCommand &cmd1,const PlayerCommand &cmd2)
    {
        ...
        for (int i=0;i<3;i++)
        {
            ...
            gameState.AI1HeroInfo[i].missleCD=MISSLE_CD;        //置CD
            ...
            if (!gameState.missileInfo[k].isReal)
            {
                ...
                gameState.missileInfo[k].power=std::max(0.0,(double)MISSILE_ORIGIN_POWER+MISSLE_DAMAGE_PER_STENTH*hero.heroTechTree.strenth);//计算伤害
        ...
        for (int i=0;i<3;i++)
        {
            ...
            gameState.AI2HeroInfo[i].missleCD=MISSLE_CD;        //置CD
	...
	void logic::HeroDeath(Status & gameState)
	{
		for(int i=0;i<3;i++)
		{
			if(gameState.AI1HeroInfo[i].currentHP<=0) //若英雄HP为非正且未死亡(后者条件很重要,以防止等待复活的英雄每次都被"死亡")
			{
				...
				if(gameState.AI1HeroInfo[i].hasPot)             //若有香锅
				{
					gameState.AI1HeroInfo[i].hasPot=false;         //拿的香锅没了
					gameState.potInfo.groundPots[gameState.potInfo.groundNumber]=gameState.AI1HeroInfo[i].heroPosition;//拿的香锅掉地上了
		...
		for(int i=0;i<3;i++)//对方
		{
			if(gameState.AI2HeroInfo[i].currentHP<=0)
			{
				...
				if(gameState.AI2HeroInfo[i].hasPot)             //若有香锅
				{
					gameState.AI2HeroInfo[i].hasPot=false;         //拿的香锅没了
					gameState.potInfo.groundPots[gameState.potInfo.groundNumber]=gameState.AI2HeroInfo[i].heroPosition;//拿的香锅掉地上了
					gameState.potInfo.groundNumber++;
				}
